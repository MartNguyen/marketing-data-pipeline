...
def load_facebook_data():
    ...
    # 3. KHỞI TẠO PIPELINE (Vùng Singapore)
    pipeline = dlt.pipeline(
        pipeline_name="fb_ads_master_pipeline_final",
    ...
    for acc_id in fb_account_ids:
        ...
        # SOURCE 1: OBJECTS (Merge dữ liệu cũ)
        ...
        # SOURCE 2: MASTER INSIGHTS (Dùng key 'ads_insights')
        master_ins = facebook_insights_source(
            account_id=acc_id, access_token=fb_access_token,
            initial_load_past_days=30, breakdowns="ads_insights", # DÙNG KEY CHUẨN TỪ LOG
        ...
        # SOURCE 3: AGE & GENDER (Dùng key 'ads_insights_age_and_gender')
        age_gender_ins = facebook_insights_source(
            account_id=acc_id, access_token=fb_access_token,
            initial_load_past_days=30, breakdowns="ads_insights_age_and_gender", # DÙNG KEY CHUẨN TỪ LOG
        ...
        # SOURCE 4: REGION (Dùng key 'ads_insights_region')
        region_ins = facebook_insights_source(
            account_id=acc_id, access_token=fb_access_token,
            initial_load_past_days=30, breakdowns="ads_insights_region", # DÙNG KEY CHUẨN TỪ LOG
            level="ad", fields=("ad_id", "date_start", "spend", "impressions", "clicks")
        )
